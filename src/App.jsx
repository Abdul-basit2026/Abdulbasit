...
const CONFIG = {
 ...
 skills: [
    ...
    { name: "Electron.js", level: 85, category: "Desktop" },
    { name: "IPC (preload/contextBridge)", level: 80, category: "Desktop" },
    { name: "Printing & Receipts", level: 80, category: "Desktop" },
    { name: "Web Development", level: 90, category: "Core" },
    { name: "Problem Solving", level: 90, category: "Core" },
    { name: "IT Support", level: 70, category: "IT" },
  ...
  experience: [
    ...
    },
    {
      ...
      technologies: ["HTML5", "CSS3+TAILWIND", "REACT-JS","Static Site Deployment"]
    },
    {
    ...
    ,
    {
      role: "Tailor Management System (Desktop)",
      company: "Self-Project",
      location: "Peshawar",
      period: "2024 - 2025",
      type: "Desktop App",
      description: "Professional Tailor Management desktop app (Electron) providing dashboard, customer management, order entry, measurement cards, printable receipts, and license activation. Offline-first with local NeDB persistence, Urdu support, and fast order/workflow UI for tailoring shops.",
      achievements: [
        "Implemented full customer & orders lifecycle with search, pagination and status workflow",
        "Built measurement forms and printable measurement cards (inches) for tailors",
        "Added printable receipts, overdue highlighting and daily/monthly revenue trend charts",
        "Integrated machine-bound license activation and local settings persistence"
      ],
      technologies: ["Electron.js", "Node.js", "@seald-io/nedb (NeDB)", "IPC (preload/contextBridge)", "HTML/CSS/JS", "Printing"]
    },
    {
      role: "Desi Restaurant Management (Desktop)",
      company: "Self-Project",
      location: "Peshawar",
      period: "2025",
      type: "Desktop App",
      description: "A performant desktop POS for restaurants built with Electron.js. Features fast order entry, printed receipts, kitchen slips, inventory tracking and daily sales summaries suitable for offline environments.",
      achievements: [
        "Implemented printable receipts and kitchen order slips",
        "Added daily sales reporting and offline-capable order storage"
      ],
      technologies: ["Electron.js", "Node.js", "@seald-io/nedb", "HTML/CSS/JS"
        ,"Printing receipts"]
    },
    {
      role: "Fast Foods POS (Desktop)",
      company: "Project",
      location: "Peshawar",
      period: "2025",
      type: "Desktop App",
      description: "Foodies Lounge POS — a low-latency Electron-based point-of-sale targeting quick-service restaurants. Implements categorized menu, fast order entry, counters, printable receipts, kitchen slips and local NeDB storage with auto-cleanup.",
      achievements: [
        "Implemented local NeDB persistence with auto-delete of old orders (7 days)",
        "Built receipt + kitchen print layouts and reprint support",
        "Added license activation (machine-bound keys) and machine ID handling",
        "Dashboard with 7-day revenue trend and top items metrics"
      ],
      technologies: ["Electron.js", "Node.js", "@seald-io/nedb (NeDB)", "IPC (preload)", "HTML/CSS/JS","Printing receipts" ]
    }
  ],
  ...
  projects: [
    ...
    },
    {
      ...
      subtitle: "Desktop Management App (Electron.js)",
      description: "Full-featured tailor management desktop app built with Electron. Features: dashboard, customers, orders, measurement entry & printable measurement cards, receipts, license activation, and local NeDB storage for offline use. Urdu UI support and fast data workflows for shop operations.",
      image: "/tailor.jpg",
      tags: ["Electron.js", "Node.js", "NeDB", "IPC", "Printing"],
      stats: { installs: "200+", customers: "50+" },
      links: {},
      featured: false,
      color: "#f59e0b"
    },
    {
      id: 5,
      title: "Shinwari Restaurant Management",
      subtitle: "Restaurant POS & Inventory (Electron.js)",
      description: "Electron-based restaurant POS featuring fast order entry, printed receipts, kitchen slips and offline-capable order storage. Includes basic sales reporting and inventory tracking.",
      image: "desi.jpg",
      tags: ["Electron.js", "Node.js", "NeDB", "POS"],
      stats: { outlets: "3+", transactions: "10k+" },
      links: {},
      featured: false,
      color: "#ef4444"
    },
    {
      id: 6,
      title: "Fast Foods POS",
      subtitle: "Foodies Lounge POS — Quick-Service POS (Electron.js)",
      description: "Full-featured quick-service POS (Foodies Lounge). Implements categorized menu, counters, fast order entry, printable customer & kitchen receipts, machine-bound license activation, and local NeDB storage with auto-cleanup of old orders.",
      image: "fast.jpg",
      tags: ["Electron.js", "Node.js", "NeDB", "IPC", "Printing"],
      stats: { installations: "150+", ordersHandled: "50k+" },
      links: {},
      featured: false,
      color: "#10b981"
    }
...
const GlobalStyles = () => (
  <style>{`
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* About */
    .about-section .section-header { margin-bottom: 0px;}
    ...
    .loader-brand { display: flex; flex-direction: column; gap: 6px; }
    .loader-title { font-weight: 800; color: var(--text); font-size: 1.15rem; }
    .loader-sub { color: var(--text-secondary); font-size: 0.9rem; }
    .loader-spinner {
      width: 56px; height: 56px; border-radius: 50%; position: relative; display: inline-block;
      background: conic-gradient(var(--gradient-1), var(--gradient-2), var(--gradient-3), var(--gradient-1));
      animation: spin 1.4s linear infinite; mask-image: radial-gradient(circle, transparent 62%, black 63%);
    }
    .loader-dots { display: flex; gap: 8px; margin-top: 6px; }
    .loader-dots span { width: 8px; height: 8px; background: var(--text-secondary); border-radius: 50%; animation: pulse 1s infinite; }
    .loader-dots span:nth-child(2) { animation-delay: 0.15s; }
    .loader-dots span:nth-child(3) { animation-delay: 0.3s; }
  `}</style>
);

// Local storage helpers for viewers/messages
const pushLocalArray = (key, item, max = 1000) => {
  try {
...
// Admin panel to view visitors and messages when visiting ?admin=1
const AdminPanel = () => {
  const [viewers, setViewers] = useState(() => readLocalArray('portfolio_viewers'));
  const [messages, setMessages] = useState(() => readLocalArray('portfolio_messages'));

  useEffect(() => {
    const onStorage = () => {
      setViewers(readLocalArray('portfolio_viewers'));
      setMessages(readLocalArray('portfolio_messages'));
    };
  ...
  const exportCsv = (arr, filename) => {
    if (!arr || !arr.length) return alert('Nothing to export');
  ...
  const clear = (key, setter) => {
    localStorage.removeItem(key); setter([]);
  };

  return (
    <section style={{ padding: 20, background: 'linear-gradient(90deg,#0b1221,#071225)', color: '#fff' }}>
      <h3>Admin — Visitors & Messages</h3>
      <div style={{ display: 'flex', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <h4>Visitors ({viewers.length})</h4>
          <div style={{ maxHeight: 240, overflow: 'auto', background: '#041025', padding: 8, borderRadius: 6 }}>
            {viewers.map((v, i) => (
              <div key={i} style={{ padding: 6, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                <div style={{ fontSize: 12, opacity: 0.8 }}>{new Date(v.timestamp).toLocaleString()}</div>
                <div style={{ fontSize: 13 }}>{v.path} — {v.title}</div>
                <div style={{ fontSize: 12, opacity: 0.7 }}>{v.ua}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 8 }}>
            <button onClick={() => exportCsv(viewers, 'viewers.csv')}>Export CSV</button>
            <button onClick={() => clear('portfolio_viewers', setViewers)} style={{ marginLeft: 8 }}>Clear</button>
          </div>
        </div>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ maxHeight: 240, overflow: 'auto', background: '#041025', padding: 8, borderRadius: 6 }}>
            {messages.map((m, i) => (
              <div key={i} style={{ padding: 6, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                <div style={{ fontSize: 12, opacity: 0.8 }}>{new Date(m.timestamp).toLocaleString()}</div>
                <div style={{ fontSize: 13 }}>{m.name} — {m.email}</div>
                <div style={{ fontSize: 13, marginTop: 6 }}>{m.message}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 8 }}>
            <button onClick={() => exportCsv(messages, 'messages.csv')}>Export CSV</button>
            <button onClick={() => clear('portfolio_messages', setMessages)} style={{ marginLeft: 8 }}>Clear</button>
          </div>
        </div>
      </div>
    </section>
...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // If a contact webhook is configured, POST the form there (Formspree / Zapier / custom server)
    if (CONFIG.contactWebhook && CONFIG.contactWebhook.trim() !== '') {
      (async () => {
        try {
          const res = await fetch(CONFIG.contactWebhook, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: form.name, email: form.email, message: form.message, page: window.location.href, ua: navigator.userAgent, timestamp: Date.now() })
          });
          if (!res.ok) throw new Error('Request failed');
          // store message locally for admin view
          try { pushLocalArray('portfolio_messages', { name: form.name, email: form.email, message: form.message, timestamp: Date.now() }, 2000); } catch (e) {}
          alert('Message sent — thank you!');
          setForm({ name: '', email: '', message: '' });
        } catch (err) {
          console.error('Contact webhook error:', err);
          alert('Failed to send via webhook — opening mail client');
...
// Loader component (spinner-only)
const Loader = () => {
  return (
    <div className="loader-overlay" role="status" aria-live="polite">
      <div className="loader-card" aria-hidden={false}>
...
const App = () => {
  ...
  const [loading, setLoading] = useState(CONFIG.loaderEnabled === true);
  const isAdmin = typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('admin') === '1';

  // Record a view event locally and optionally POST to configured webhook
  useEffect(() => {
    if (loading) return;
    const evt = { path: window.location.pathname, title: document.title, ua: navigator.userAgent, timestamp: Date.now() };
    try { pushLocalArray('portfolio_viewers', evt, 2000); } catch (e) {}
    if (CONFIG.viewWebhook && CONFIG.viewWebhook.trim() !== '') {
      (async () => {
        try {
          await fetch(CONFIG.viewWebhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(evt) });
        } catch (e) { console.warn('viewWebhook failed', e); }
      })();
    }
  }, [loading]);
  ...
  // Loader effect: hide when window 'load' fires or after fallback timeout
  useEffect(() => {
    if (!CONFIG.loaderEnabled) return;
    let finished = false;
    const minDelay = CONFIG.loaderMinMs || 700;
    ...
    if (document.readyState === 'complete') {
      ...
      const onLoad = () => { if (!finished) hide(); };
      window.addEventListener('load', onLoad);
      const fallback = setTimeout(() => { if (!finished) hide(); }, 4000);
  ...
  return (
    <>
      <GlobalStyles />
      {loading && <Loader />}
      <div style={{ filter: loading ? 'blur(2px) brightness(0.9)' : 'none', pointerEvents: loading ? 'none' : 'auto' }}>
